...
for i  = 1 : rule
EV0_mu1{i}  = LMIfunc(A{i},Ad{i},0,   mu1,h,mu1,mu2,P0,P1,Q1,Q2,R11,R12,R21,R22,R31,R32,L{i},Y1til,Y2til,K1,K2,K3,n);
EVh2_mu1{i} = LMIfunc(A{i},Ad{i},h/2, mu1,h,mu1,mu2,P0,P1,Q1,Q2,R11,R12,R21,R22,R31,R32,L{i},Y1til,Y2til,K1,K2,K3,n);
EVh_mu1{i}  = LMIfunc(A{i},Ad{i},h,   mu1,h,mu1,mu2,P0,P1,Q1,Q2,R11,R12,R21,R22,R31,R32,L{i},Y1til,Y2til,K1,K2,K3,n);

EV0_mu2{i}  = LMIfunc(A{i},Ad{i},0,   mu2,h,mu1,mu2,P0,P1,Q1,Q2,R11,R12,R21,R22,R31,R32,L{i},Y1til,Y2til,K1,K2,K3,n);
EVh2_mu2{i} = LMIfunc(A{i},Ad{i},h/2, mu2,h,mu1,mu2,P0,P1,Q1,Q2,R11,R12,R21,R22,R31,R32,L{i},Y1til,Y2til,K1,K2,K3,n);
EVh_mu2{i}  = LMIfunc(A{i},Ad{i},h,   mu2,h,mu1,mu2,P0,P1,Q1,Q2,R11,R12,R21,R22,R31,R32,L{i},Y1til,Y2til,K1,K2,K3,n);
end
...
for i = 1 :rule
    LMIcon1{i} = [F0_mu1{i},         sqrt(2)*X1'*Y2til{1};
                 sqrt(2)*(X1'*Y2til{1})',    -(R12T + R32T)];  
    LMIcon2{i} = [F0_mu2{i},         sqrt(2)*X1'*Y2til{2};
                 sqrt(2)*(X1'*Y2til{2})', -(R12T + R32T)];


    LMIcon3{i} = [h^2* F2_mu1{i} + h* F1_mu1{i} + F0_mu1{i}, sqrt(2)*X2'*Y1til{1}';
                  sqrt(2)*(X2'*Y1til{1}')'             , -(R11T + R21T)];                     
    LMIcon4{i} = [h^2* F2_mu2{i} + h* F1_mu2{i} + F0_mu2{i}, sqrt(2)*X2'*Y1til{2}';
                  sqrt(2)*(X2'*Y1til{2}')'             ,  -(R11T + R21T)];


    LMIcon5{i} = [c{i}*h* F1_mu1{i} + 2* F0_mu1{i},  sqrt(4-2*c{i})*X1'*Y2til{1},  sqrt(2*c{i})*X2'*Y1til{1}' ;
                  (sqrt(4-2*c{i})*X1'*Y2til{1})',  -(R12T + R32T)               ,  zeros(3*n)                   ;
                  (sqrt(2*c{i})*X2'*Y1til{1}')' ,   zeros(3*n)                  ,  -(R11T + R21T) ];


    LMIcon6{i} = [c{i}*h* F1_mu2{i} + 2* F0_mu2{i},  sqrt(4-2*c{i})*X1'*Y2til{2},  sqrt(2*c{i})*X2'*Y1til{2}' ;
                  (sqrt(4-2*c{i})*X1'*Y2til{2})',  -(R12T + R32T)                 ,  zeros(3*n)                   ;
                  (sqrt(2*c{i})*X2'*Y1til{2}')' ,   zeros(3*n)                    ,  -(R11T + R21T) ];


    LMIcon7{i} = [2*c{i}*h^2* F2_mu1{i} + (1 + c{i})*h* F1_mu1{i} + 2*F0_mu1{i}   ,  sqrt(2-2*c{i})*X1'*Y2til{1},  sqrt(2+2*c{i})*X2'*Y1til{1}' ;
                  (sqrt(2-2*c{i})*X1'*Y2til{1})'                                ,  -(R12T + R32T)               ,  zeros(3*n)                   ;
                  (sqrt(2+2*c{i})*X2'*Y1til{1}')'                                 ,   zeros(3*n)                  ,  -(R11T + R21T) ];
    
    LMIcon8{i} = [2*c{i}*h^2* F2_mu2{i} + (1 + c{i})*h* F1_mu2{i} + 2*F0_mu2{i}   ,  sqrt(2-2*c{i})*X1'*Y2til{2},  sqrt(2+2*c{i})*X2'*Y1til{2}' ;
                  (sqrt(2-2*c{i})*X1'*Y2til{2})'                                ,  -(R12T + R32T)               ,  zeros(3*n)                   ;
                  (sqrt(2+2*c{i})*X2'*Y1til{2}')'                                 ,   zeros(3*n)                  ,  -(R11T + R21T) ];
...
for i = 1 : rule
    constraints = [constraints, LMIcon1{i}<= -epsilon*eye(13*n), 
                                LMIcon2{i}<= -epsilon*eye(13*n), 
                                LMIcon3{i}<= -epsilon*eye(13*n), 
                                LMIcon4{i}<= -epsilon*eye(13*n), 
                                LMIcon5{i}<= -epsilon*eye(16*n), 
                                LMIcon6{i}<= -epsilon*eye(16*n), 
                                LMIcon7{i}<= -epsilon*eye(16*n), 
                                LMIcon8{i}<= -epsilon*eye(16*n), 
                                 L{i} >= epsilon*eye(n)];
end
...
function [EE]=LMIfunc(A,Ad,pit,pit_dot,h,mu1,mu2,P0,P1,Q1,Q2,R11,R12,R21,R22,R31,R32,L,Y1til,Y2til,W1,W2,W3,n)
...
phi1 =  R11T + (1-alpha)*(R11T+R21T);
phi2 =  (1-alpha)*Y1dot + alpha*Y2dot ;
phi3 =  R12T + alpha*(R12T+R32T);
